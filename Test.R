...
trainFile <- ifelse(nodename=="bigtumor",("/home/tumor/MLExperimental/walmart/data/trainWide.rdata"),
                    ifelse(os=="Darwin",
                           ...
                           ("/media/3TB/kag/walmart/data/trainWide.rdata")))
load(trainFile)
...
testFile <- ifelse(nodename=="bigtumor",("/home/tumor/MLExperimental/walmart/data/testWide.rdata"),
                   ifelse(os=="Darwin",
                          ...
                          ("/media/3TB/kag/walmart/data/testWide.rdata")))
load(testFile)
...
# trainCounts <- ddply(train,.(TripType,VisitNumber,Weekday,DepartmentDescription),function(x){data.frame(PurchaseCount=sum(x[x$ScanCount>0,]$ScanCount),ReturnCount=sum(x[x$ScanCount<0,]$ScanCount))})
