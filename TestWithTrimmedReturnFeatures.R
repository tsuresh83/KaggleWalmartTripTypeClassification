...
startTime <- Sys.time()
factorsAsIntegers <- F
scriptName <- "WalmartTrimReturnFeatures"
set.seed(13)
os <- Sys.info()[["sysname"]]
...
for(x in grep("Count|Unique",missTestDeptNames)){
  testWideNormalized <- cbind(testWideNormalized,0)
}
colnames(testWideNormalized)[(ncolsTestBeforeAddition+1):ncol(testWideNormalized)] <- missTestDeptNames[grep("Count|Unique",missTestDeptNames)]
